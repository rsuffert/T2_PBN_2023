...
{
	// 1. calculate the difference between the Xs and the Ys
	uint8_t dx = x2 - x1;
	uint8_t dy = y2 - y1;

	// 2. determine how many steps (points) it will take to draw the line
	uint8_t steps = abs(dx) > abs(dy)? abs(dx) : abs(dy);

	// 3. calculate the x-increment and the y-increment for each step
    float x_inc = (float)dx / steps;
    float y_inc = (float)dy / steps;

	// 4. inicialize current values of x and y
    float xLine = x1;
    float yLine = y1;

	// 5. draw current point
    nokia_lcd_set_pixel((uint8_t)xLine, (uint8_t)yLine, 1);

	for (uint8_t i=0; i<steps; i++) // for each step
	{
		// increment current values of x and y
		xLine += x_inc;
		yLine += y_inc;
		// draw the new point
        nokia_lcd_set_pixel((uint8_t)xLine, (uint8_t)yLine, 1);
...
void nokia_lcd_draw_circle(uint8_t centerX, uint8_t centerY, uint8_t radius)
{
    ...
    uint8_t error = 1 - radius;

    // loop for draing the circle
    while (x >= y)
    {
        // draw the 8 symmetric points
        nokia_lcd_set_pixel(centerX + x, centerY + y, 1);
        nokia_lcd_set_pixel(centerX + y, centerY + x, 1);
        nokia_lcd_set_pixel(centerX - y, centerY + x, 1);
        nokia_lcd_set_pixel(centerX - x, centerY + y, 1);
        nokia_lcd_set_pixel(centerX - x, centerY - y, 1);
        nokia_lcd_set_pixel(centerX - y, centerY - x, 1);
...
{
    uint8_t bottomLeftX = topLeftX;
    uint8_t bottomLeftY = bottomRightY;
    uint8_t topRightX   = bottomRightX;
    uint8_t topRightY   = topLeftY;

	nokia_lcd_draw_line(topLeftX, topLeftY, topRightX, topRightY); // up
	nokia_lcd_draw_line(bottomLeftX, bottomLeftY, bottomRightX, bottomRightY); // bottom
	nokia_lcd_draw_line(topLeftX, topLeftY, bottomLeftX, bottomLeftY); // left
	nokia_lcd_draw_line(topRightX, topRightY, bottomRightX, bottomRightY); // right
}
